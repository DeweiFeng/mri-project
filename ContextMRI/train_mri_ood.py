...
def collate_fn(examples):
    ...
    ##
    is_ood_flags = [example.get("is_ood", False) for example in examples]
    ##
    ...
    ##
    mask = torch.tensor(is_ood_flags, dtype=torch.bool)
    if mask.any():
        flat = pixel_values[mask].reshape(pixel_values[mask].shape[0], -1)
        p99 = torch.quantile(flat.abs(), 0.99, dim=1)
        p99 = torch.where(p99 > 0, p99, torch.ones_like(p99))
        shape = [p99.shape[0]] + [1] * (pixel_values.ndim - 1)
        scale = p99.view(*shape)
        pixel_values[mask] = pixel_values[mask] / scale
    ##
    ...
    batch = {
        ...
        ##
        "is_ood": torch.tensor(is_ood_flags, dtype=torch.bool),
        ##
    }
    return batch
...
class LabeledDataset(torch.utils.data.Dataset):
    """Wraps dataset and tags samples with 'is_ood'"""
    def __init__(self, base_ds, is_ood: bool):
        self.base = base_ds
        self.is_ood = is_ood
    def __len__(self):
...
def main(args):
    ...
    ##
    train_dataset = LabeledDataset(train_dataset, is_ood=False)
    ##

    ##
    if args.use_ood:
        if args.cardio_data_root is None or args.mri_metadata_dir_cardio is None:
            raise ValueError("Must set --cardio_data_root and --mri_metadata_dir_cardio when using OOD")
        cardio_dataset = MRIDataset(
            metadata_file_knee=args.mri_metadata_dir_knee,
            metadata_file_brain=args.mri_metadata_dir_cardio,
            train=True,
            data_root=args.cardio_data_root,
        )
        cardio_dataset = LabeledDataset(cardio_dataset, is_ood=True)
        train_dataset = ConcatDataset([train_dataset, cardio_dataset])
    ##
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                if args.enable_condition_emb:
                    ...
                    # )
                    cond_embeddings = accelerator.unwrap_model(unet).condition_emb(metadata_list, dtype=weight_dtype, device=accelerator.device)
                ...
                t_in = torch.randint(0, num_T, (bsz,), device=model_input.device)
                t_ood = torch.randint(3*num_T//4, num_T, (bsz,), device=model_input.device)
                mask   = batch["is_ood"].to(model_input.device)
                timesteps = torch.where(mask, t_ood, t_in)
                ##
...
def parse_args(input_args=None):
    ...
    ##
    parser.add_argument(
        "--cardio_data_root", type=str, default=None,
        help="Root directory of cardiology .npy data."
    )
    parser.add_argument(
        "--mri_metadata_dir_cardio", type=str, default=None,
        help="CSV folder for cardiology metadata."
    )
    parser.add_argument(
        "--use_ood", action="store_true",
        help="Include out‑of‑distribution cardiology data in training."
    )
    ##
    parser.add_argument(
        "--rank",
...
accelerate launch --num_processes=2 --num_machines=1 train_mri_ood.py \
  ...
  --mri_metadata_dir_cardio metadata_card_train.csv \
  --use_ood \
  --output_dir ../output_ood \

...
import functools
from mri.fastmri_utils import fft2c_new, ifft2c_new
...
def CG(A, b, x, n_inner=5, eps=1e-5, save_dir=None, save=False):
    r = b - A(x)
    p = r.clone()
    rsold = torch.matmul(r.view(1, -1), r.view(1, -1).T)

    if save and save_dir is not None:
        plt.imsave(os.path.join(save_dir, f"origin.png"), np.abs(clear(x)), cmap="gray")
    ...
    for i in range(n_inner):
        ...
        if save and save_dir is not None:
            plt.imsave(
                os.path.join(save_dir, f"{i:03d}.png"), np.abs(clear(x)), cmap="gray"
            )
...
def window(img, minv=0.0, maxv=6.0):
    img -= minv
    img /= maxv - minv
    img = np.clip(img, 0, 1)
    return img
...
def patient_wise_min_max(img):
    ...
    min_max_scaled[
        min_max_scaled > min_max_scaled_mean + std_upper * min_max_scaled_std
    ] = 1
...
def image_grid_gray(x, size=32):
    ...
    w = int(np.sqrt(img.shape[0]))
    img = (
        img.reshape((w, w, size, size))
        .transpose((0, 2, 1, 3))
        .reshape((w * size, w * size))
    )
    return img
...
def nchw_to_kspace(tensor):
    ...
    if tensor.shape[1] == 1:
        ...
        s = tensor.shape
    tensor = tensor.view(size=(s[0], s[1] // 2, 2, s[2], s[3])).permute(
        dims=(0, 1, 3, 4, 2)
    )
    return tensor
